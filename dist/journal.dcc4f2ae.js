...
if (favorites.length === 0) {
...
} else favorites.forEach((movie)=>{
    const wrapper = document.createElement("div");
    wrapper.className = "p-20 rounded-lg shadow-md mb-4 flex flex-col items-left bg-zinc-950";
    ...
    movieDescription.className = "text-lg w-1/2 text-gray-400 mb-4 text-left content-center justify-normal ";
    ...
    for(let i = 1; i <= 5; i++){
        ...
        star.textContent = "\u2605";
        star.className = "text-gray-400 cursor-pointer text-2xl transition-all hover:text-yellow-400";
        star.dataset.value = i;
        // Highlight stars based on saved rating
        if (movie.rating && i <= movie.rating) star.classList.add("text-yellow-400");
    ...
    reviewInput.className = "w-4/12 p-3 border rounded mb-2 focus:outline-none text-sm";
    ...
    submitButton.className = "bg-blue-500 text-white px-4 py-2 rounded shadow-md hover:bg-blue-600 w-3/12";
    ...
    submitButton.addEventListener("click", ()=>{
        ...
        } else alert("Please write a review before submitting.");
    });
